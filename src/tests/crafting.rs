...
fn the_shipped_ruleset_tears_a_trophy_down_and_sews_its_own() {
    ...
    // Материал для пошива кладём явно: с §12.129 заявка режется наличным, и без
    // этого отказ ниже говорил бы про пустой склад, а проверяем мы **ворота
    // технологии**. Ткани разбор дал две, а рецепту нужно три и деталь.
    let part = 1;
...
///
/// Считается это **по кучам, а не по складу** (§12.129): станок так и
/// снабжается — материал к нему возят ногами и берут откуда угодно, включая пол
/// (§12.69, §12.102). До §12.129 мерка была складской, и довод у неё был про
/// соседнюю кнопку, которая склада не проверяла вовсе; теперь ворота у обеих
/// одни.
#[test]
fn salvage_never_orders_more_than_there_is() {
    ...
    sim.put_item(1, 1, PART, 2); // на полу: станку это материал (§12.102)
