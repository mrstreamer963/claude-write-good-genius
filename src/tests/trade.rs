...
/// **Вторая ловушка, сторона вторая: выставляется только складское** (§12.69).
///
/// Излишек считает вся база, включая пол, — а продать наружу можно лишь
/// учтённое. Без второго числа правило просило бы больше, чем на складе,
/// `trade` отклонял бы заявку целиком, и куча, ждущая уборки, останавливала бы
/// продажу вовсе.
#[test]
fn a_threshold_offers_only_what_storage_holds() {
    ...
    assert_eq!(
        sim.sales(),
        vec![(f, 0, 5)],
        "излишек шесть, но выставлено пять — больше на складе нет",
    );
    ...
    assert_eq!(
        sim.item_total(0),
        4,
        "убранное с пола ушло следующей сделкой"
    );
