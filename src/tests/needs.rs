...
/// Стеллаж — стена (§12.142): дорога к лежанке его **обходит**, а не пересекает.
/// До §12.142 полку проходили насквозь, и на боевой партии это давало качели —
/// шаг на полку, `clear_solids` гонит обратно, шаг на полку (трейс от 6 августа).
/// Теперь такой клетки в маршруте нет вовсе, и качелям неоткуда взяться.
#[test]
fn a_nap_walks_around_a_rack() {
    ...
    sim.force_tile(4, 1, 2); // два стеллажа подряд, но нижний ряд свободен
    sim.set_needs(1000, 300, 1);
    sim.set_energy("a", 800);
    ...
    assert_eq!(
        sim.pos_of("a"),
        (5, 1),
        "дошёл до лежанки в обход, а не застрял у полок"
    );
...
/// Выносливость замедляет расход бодрости — кто дольше держится, тот чаще в
/// поле (§12.70).
///
/// Реализовано **расходом, а не потолком**: пороги остаются абсолютными, сутки
/// считаются одной формулой на всех, и выносливый именно дольше бодрствует, а
/// не дольше отсыпается.
#[test]
fn stamina_slows_the_burn() {
    let mut sim = sim_from(&["#####", "#a.b#", "#####"]);
    sim.set_needs(1000, 0, 1);
    sim.set_drain(10);
    sim.set_stat_steps("stamina", &[3, 5, 7, 9]);
    sim.set_stat("a", 0, 9); // ступень 4 — расход 6
    sim.set_stat("b", 0, 1); // ступеней нет — расход 10

    sim.tick_n(50);
    assert_eq!(
        sim.energy_of("a"),
        1000 - 50 * 6,
        "выносливый горит медленнее"
    );
    assert_eq!(sim.energy_of("b"), 1000 - 50 * 10, "а обычный — по базе");
}

/// Ниже единицы расход не опускается никогда: кот, который не устаёт вовсе, не
/// уснёт, а бодрость — это плата за котовремя, а не необязательный налог.
#[test]
fn the_burn_never_reaches_zero() {
    let mut sim = sim_from(&["####", "#a.#", "####"]);
    sim.set_needs(100, 0, 1);
    sim.set_drain(2);
    sim.set_stat_steps("stamina", &[1, 2, 3, 4]);
    sim.set_stat("a", 0, 9); // ступень 4 при базе 2 — ушло бы в минус

    sim.tick_n(10);
    assert_eq!(sim.energy_of("a"), 90, "расход упёрся в единицу");
}
