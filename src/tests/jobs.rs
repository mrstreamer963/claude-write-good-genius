...
/// Ничью решает `id` кота, а не порядок сущностей в ECS.
///
/// Два кота на равном расстоянии от одного чертежа — ситуация рядовая, и
/// раньше побеждал тот, кто раньше лежит в таблице архетипа. Это порядок
/// вставок (§11): он воспроизводим внутри прогона, но у загруженного из
/// снимка мира свой, и та же база раздала бы работу иначе (§12.45).
///
/// Схемы отличаются **только порядком появления котов** — `sim_from` спавнит
/// их слева направо. Логически это один и тот же мир, и решение обязано
/// совпасть.
#[test]
fn a_tie_is_broken_by_id_not_by_spawn_order() {
    const OTHER: i32 = 2;
    ...
    for (rows, where_a) in [
        ...
        // Чертёж между ними: обоим до места работы ноль шагов.
        sim.add_blueprint(2, 1, OTHER);
        sim.tick_n(1);

        assert!(
            sim.has_assignment("a"),
            "чертёж достался 'a' (схема {rows:?}, 'a' стоит в x={where_a})",
        );
        assert!(!sim.has_assignment("b"), "и только ему");
    }
}
