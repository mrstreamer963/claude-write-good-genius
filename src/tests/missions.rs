...
/// Стартовая застройка даёт **ровно один** гараж и **ровно одну** рацию
/// (§12.59, §12.152).
///
...
///
/// Рация одна по прежнему доводу (§12.59): вторую надо хотеть, — но слота она
/// больше не даёт, поэтому и стережётся отдельно от гаража.
#[test]
fn the_shipped_ruleset_starts_with_one_relay_node() {
    ...
    assert_eq!(sim.relay_count(), 1, "и рация в зале одна");
