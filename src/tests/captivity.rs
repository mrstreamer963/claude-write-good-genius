...
#[test]
fn captivity_does_not_depend_on_the_second_squad() {
    let mut sim = gate_world(
        &["##########", "#a.b.c.d.#", "#........#", "##########"],
        (2, 1),
    );
    ...
    assert!(
        sim.is_captive("a"),
        "и это не спасло первого от плена: база всё та же",
    );
}
...
fn the_shipped_ruleset_can_always_come_for_its_own() {
    ...
    // Логово открыто (известность плюс доверие Полиции, §12.43), но силы
    // стартовой пары на него нет — а проверяем мы здесь именно обратимость плена.
    sim.set_fame(60);
    sim.set_standing(0, 30);
    assert!(sim.launch(3, squad(&["excellent", "sp2"])), "ушли в логово");
    sim.tick_n(1200);
