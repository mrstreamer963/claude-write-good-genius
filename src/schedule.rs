...
///
/// Двенадцать раздатчиков берут котов из одного пула свободных, поэтому их порядок —
...
///  11. `assign_relay` — дежурство на узле связи (§12.60). После всех работ,
...
///  12. `assign_nap` — дремота (§12.52). После всех, потому что раздаёт она не
