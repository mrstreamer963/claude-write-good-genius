...
///
/// Восемь раздатчиков берут котов из одного пула свободных, поэтому их порядок —
/// это и есть приоритет работ (§12.15, §12.16, §12.20, §12.26, §12.30, §12.36):
///   1. `assign_eat` — еда. Даже раньше отдыха: ходка за пайком короткая, а сон
...
///   2. `assign_rest` — отдых. Иначе уставшего кота тут же уводит работа.
///   3. `assign_equip` — экипировка. Сразу за отдыхом и до всех работ: на базе
///      работа есть всегда, и одетым отряд не уходил бы никогда (§12.34).
///      Ходка короткая и разовая — комплект добирается один раз.
///   4. `assign_hauls` — подвоз на площадки. Иначе бригада разбирает уже
///      обеспеченные чертежи, а за ломом для остальных никто не идёт, и база
///      встаёт тем вернее, чем больше на ней работы.
///   5. `assign_research` — наука. Раньше стройки: за тему уже заплачено
...
///   6. `assign_craft` — производство. Там же и по той же причине: заказ игрок
///      разметил явно и поштучно, а чертежей на базе всегда больше (§12.30).
///   7. `assign_jobs` — стройка и снос.
///   8. `assign_tidy` — уборка пола. Последняя: подбирать мусор, пока стоит
