...
pub(crate) struct EntitySnap {
    ...
    /// Кот лежит и заживает (в пути к койке — ещё нет). Рядом со `sleeping` и
    /// по той же причине: игрок должен видеть, почему тот не работает.
    pub(crate) healing: bool,
    /// Кот лечит раненого — вторая половина той же картинки: медик занят не
    /// бездельем, а чужими ранами.
    pub(crate) treating: bool,
