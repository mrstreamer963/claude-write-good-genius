...
impl Sim {
    ...
    pub fn set_target(&mut self, unit_id: &str, x: i32, y: i32) -> bool {
        ...
        // А приписку к парте приказ **снимает** (§12.84), в отличие от приписки
        // к узлу связи. Обе адресны, но узел зовёт кота только на время чужой
        // вылазки и работать не мешает, а парта — это «стой тут и учись»: два
        // распоряжения «иди туда» и «сиди там» противоречат друг другу, и без
        // снятия кот вернулся бы за парту следующим же тиком. Зеркально к тому,
        // как `teach` снимает `Order`. Отдельной команды отмены учёбе поэтому
        // не нужно: приказ «иди туда» и есть она.
        self.world.entity_mut(entity).remove::<Enrolled>();
