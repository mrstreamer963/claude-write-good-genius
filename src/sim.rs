...
impl Sim {
    ...
    pub fn launch_node(&mut self, def: usize, x: i32, y: i32) -> bool {
        ...
        // Отправили этот узел **на другой** заказ — правило автовылазки снято
        // (§12.72). Оно повторяет решение игрока, а игрок только что принял
        // другое: правило, пережившее явный приказ, вернуло бы отряд «сам
        // собой» тем же тиком, каким тот дошёл до базы, — то есть коты
        // вернулись бы с назначенной вылазки и тут же исчезли снова.
        //
        // Сравнение с `def` здесь не деталь, а вся граница: правило само уходит
        // через эту же кнопку (`run_auto_raids`), но всегда со **своим**
        // заказом, — значит снять себя оно не может. Ручная отправка на тот же
        // заказ правила тоже не трогает: другого решения игрок не принимал.
        if self
            .world
            .resource::<AutoRaids>()
            .of(x, y)
    ...
    ///
    /// С §12.75 сам по себе к рации никто не садится, поэтому «уступает» тут
