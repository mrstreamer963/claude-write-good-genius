...
use crate::hauling::{on_base_counts, plan_spend, stored_counts};
...
impl Sim {
    ...
    /// связи). Близнец `orders_running`, и намеренно: слот занимает сама
    /// миссия, а не кто-то за неё. У торговли близнеца больше нет — там слот
    /// стал местом, и свободу считает `free_post_cell` (§12.68).
...
impl Sim {
    ...
    ///
    /// Порог висит на паре **фракция + предмет**, а не на одном предмете: у
    /// продажи есть адресат, и выбирает его игрок — тем же кликом, каким продал
    /// бы вручную (§12.64: правило видно там же, где стоит команда). «Продать
    /// тому, кто даёт больше» ядру вычислить нетрудно — курс чистая функция тика
    /// (§12.44), — но такое правило обыгрывало бы игрока на его же расписании и
    /// торговало бы с той стороной, от которой он держится подальше (§12.43).
    ///
    ...
    ///
    /// **Считает оно ровно одно — сколько лишнего**, и здесь встречаются обе
    /// формы брони (§12.50), которые нельзя путать:
    ///   * «сколько у базы есть сверх порога» — это кучи и лапы (`on_base_of`,
    ///     общий с порогом производства) минус `owed_for_sale`, **без скидки на
    ///     лапы**: носильщик считается в `have`, и вторая скидка засчитала бы
    ///     его дважды, то есть правило выставило бы на продажу уже проданное;
    ///   * «сколько из этого можно выставить прямо сейчас» — это склад минус
    ///     `booked_for_sale`, то самое выражение, которым отвечает `trade`.
    ///
    /// Заявка идёт по меньшему из двух. Без второго числа правило просило бы
    /// больше, чем на складе, — `trade` отклонял бы заявку целиком, и лом, ждущий
    /// уборки, останавливал бы продажу вовсе. Без первого — считало бы излишком
    /// то, что уже обещано покупателю.
    ///
    ...
    fn run_auto_sales(&mut self) {
        ...
        // Обе брони приезжают парами «предмет, штук» — читаются одинаково.
        let of = |list: &[(usize, i32)], item: usize| {
            list.iter()
                .find(|&&(i, _)| i == item)
                .map_or(0, |&(_, n)| n)
        };
        for (faction, item, keep) in rules {
            // Ячейка кончилась — дальше по списку идти незачем: `trade` отклонил
            // бы всех подряд, а трейс наполнился бы отказами (§12.45).
            if self.free_post_cell().is_none() {
                break;
            }
            let owed = of(&self.owed_for_sale(), item);
            let surplus = self.on_base_of(item) - owed - keep;
            if surplus <= 0 {
                continue;
            }
            let booked = of(&self.booked_for_sale(), item);
            let count = surplus.min(self.in_storage(item) - booked);
            if count <= 0 {
                continue; // излишек есть, но он ещё на полу или в лапах
            }
            self.trade(faction, item, count, false);
        }
    }

    /// Сколько предмета у базы **всего**: кучи где угодно плюс лапы (§12.65).
    ///
    /// Считает это `hauling::on_base_counts` — одно место на порог производства
    /// и на порог продажи, потому что «сколько у базы есть» обязано значить одно
    /// и то же у обоих (§12.53).
    fn on_base_of(&mut self, item: usize) -> i32 {
        let mut piles = self.world.query::<&Stack>();
        let mut loads = self.world.query::<&Carrying>();
        on_base_counts(piles.iter(&self.world), loads.iter(&self.world))
            .get(item)
            .copied()
            .unwrap_or(0)
    }
    ...
    /// кто считает добро базы **вместе с лапами**, — шапка (§12.69) и порог
    /// автопродажи (§12.87). Скидка на лапы там засчитала бы носильщика дважды:
    /// один раз как «есть на базе», второй — как «уже не обещано». Тем и
    /// отличается от `booked_for_sale`: та отвечает на вопрос «сколько нельзя
    /// брать **из куч**», и лапы из неё вычтены потому, что из куч это уже взято.
    ...
    pub fn snapshot(&mut self) -> Result<JsValue, JsValue> {
        ...
        let prices: Vec<PriceSnap> = {
            ...
            let selling = self.world.resource::<Selling>();
            let mut out = Vec::new();
