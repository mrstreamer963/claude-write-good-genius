...
impl Sim {
    ...
    pub fn start_craft(&mut self, def: usize, count: i32) -> bool {
        ...
        // нехватка материала — законное ожидание (§12.30): лом придёт сам, с
        // уборки и с вылазок. У разбора это не ожидание, а **правило**: заказ
        // на пять комбинезонов при одном на базе съедал бы каждый следующий
        // трофей молча — ровно то, в чём §12.114c отказала порогу, только без
        // числа на экране и без места, где его снять. Поэтому заявка режется
        // наличным: сколько трофеев есть, столько и разберут.
        let count = if rule.salvage {
            count.min(self.salvage_room(&rule.cost))
        } else {
            count
        };
        if count <= 0 {
    ...
    /// Сколько штук разбора база потянет прямо сейчас (§12.114d): **учтённое** —
    /// склад минус бронь продажи, — минус обещанное открытым заказам.
    ///
    /// Считается по **складу**, а не по всем кучам, хотя подвоз возит и с пола
    /// (§12.69, §12.102). Довод не бухгалтерский, а про то, что игрок видит: в
    /// строке окна разбор стоит **вплотную к «Произвести»**, а тот меряет склад
    /// (`storage_covers`), и главное число строки — тоже склад (§12.53). Серый
    /// «+N» валяющегося читают через раз, и «разобрать могу больше, чем
    /// произвести из того же» выглядит поломкой соседней кнопки. Валяющееся не
    /// пропадает — его увезёт уборка, и число вырастет само.
    ///
    ...
    fn salvage_room(&mut self, cost: &[(usize, i32)]) -> i32 {
        ...
        cost.iter()
            .map(|&(item, per)| {
                // Тот же счёт, каким отвечает `storage_covers` соседней кнопке:
                // склад минус бронь. Разойдись они — в одной строке стояли бы
                // два разных ответа на «сколько у меня этого есть».
        ...
        let mut recipes = Vec::new();
        {
            ...
            for (def, rule) in rules.iter().enumerate() {
                recipes.push(RecipeSnap {
                    ...
                    room: rule.salvage.then(|| self.salvage_room(&rule.cost)),
                });
            }
        }
