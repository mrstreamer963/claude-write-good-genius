...
impl Sim {
    ...
    ///
    /// Заказ, которого коснулся игрок, становится ручным (§12.65): иначе
    /// правило-порог тем же тиком срезало бы добавленное обратно к недостаче.
    ///
    /// Вернёт false, если рецепта нет, счётчик неположителен, не хватает
    /// технологий или свободной ячейки нет и добавить штуки некуда.
    pub fn start_craft(&mut self, def: usize, count: i32) -> bool {
        ...
        // Свободная ячейка есть — заказ встаёт в неё, и это главное, ради чего
        // §12.96 и писалась: второй заказ на тот же рецепт занимает второй
        // станок, а не ждёт первого.
        let Some(cell) = self.free_shop_cell() else {
            // Ячеек не осталось. Добавляем штук к своему первому по карте
            // заказу — так работала любая повторная заявка до §12.96, и на
            // базе с одной мастерской это по-прежнему единственная ветка.
    ...
    pub fn snapshot(&mut self) -> Result<JsValue, JsValue> {
        ...
        let shop_free = self.free_shop_cell().is_some();
