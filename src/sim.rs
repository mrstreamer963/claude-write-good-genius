...
impl Sim {
    ...
    /// Сколько предмета лежит **в кучах** — на складе и на полу разом (§12.129).
    ///
    /// Это ровно то, что подвоз умеет привезти на станок (§12.69, §12.102):
    /// лапы и надетое сюда не идут — они не кучи, и заказ, считающий надетое,
    /// раздевал бы отряд перед вылазкой.
    fn piles_of(&mut self, item: usize) -> i32 {
...
impl Sim {
    ...
    /// Сколько штук база потянет прямо сейчас (§12.114d, §12.129): **все кучи**
    /// минус бронь продажи и минус обещанное открытым заказам.
    ///
    /// Считается по кучам, а не по складу, потому что станок так и снабжается:
    /// материал к нему возят ногами и **берут откуда угодно, включая пол**
    /// (§12.69, §12.102). Мерить складом значило бы отклонять заказ, который
    /// коты выполнили бы, — при полном полу лома и пустом складе.
    ///
    /// До §12.129 мерка была складской, и довод у неё был про соседнюю кнопку:
    /// «Разобрать» стоял вплотную к «Произвести», а тот склад не проверял вовсе
    /// (§12.114d), и «разобрать могу больше, чем произвести из того же»
    /// читалось поломкой. Теперь ворота у обеих кнопок одни, и расходиться им
    /// нечем.
    ///
    ...
    fn craft_room(&mut self, cost: &[(usize, i32)]) -> i32 {
        ...
        cost.iter()
            .map(|&(item, per)| {
                ...
                // станок (§12.102): кучи целиком, минус чужие обещания.
                let free = self.piles_of(item) - at(&booked, item) - at(&promised, item);
