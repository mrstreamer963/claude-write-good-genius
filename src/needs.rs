...
//!
//! Бодрость тратится за каждый тик бодрствования — **по ставке того, чем кот
//! занят** (§12.99: работа дороже ходьбы, ходьба дороже простоя, лежащий не
//! платит вовсе) — и возвращается сном. Отдых —
...
use crate::movement::{Busy, JOB_NAP, JOB_REST};
...
/// Ставка расхода: чем кот занят прямо сейчас (§12.99).
///
/// Состояний ровно столько, сколько различает карточка кота (`Busy`), и это
/// не совпадение: ставку выбирает то самое слово, которое игрок читает, —
/// второго перечня занятости ради расхода не заводится (§12.52, инвариант 7).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub(crate) enum Toil {
    /// Кот лежит — спит или дремлет. Не тратит вовсе.
    Lying,
    /// Кот в пути: к делу, к лежанке, по приказу игрока.
    Walking,
    /// Кот стоит, и дела у него нет (невыполнимый приказ — тоже простой).
    Idle,
    /// Кот за делом.
    Working,
}
...
impl Toil {
    /// Что кот делает — из того же разбора задач, что и подпись в панели
    /// (§12.41). Порядок веток и есть лестница: **идущий спать ещё идёт**, а
    /// лежащий уже не работает.
    pub(crate) fn of(busy: &Busy) -> Self {
        if busy.moving {
            Toil::Walking
        } else if busy.job == JOB_REST || busy.job == JOB_NAP {
            Toil::Lying
        } else if busy.idle {
            Toil::Idle
        } else {
...
/// Сколько бодрости кот тратит за тик — по тому, чем занят (§12.70, §12.99).
///
/// Ступень «Выносливости» вычитается из **действующей** ставки, а не из
/// единственной, — и смысл её от этого не меняется: выносливый **дольше
/// держится**, а не дольше отсыпается. Пороги при этом остаются абсолютными, а
/// сутки — одной формулой (`day = max / drain`, §12.46), просто считает она
/// теперь день кота, который весь день работал. Потолок от параметра потребовал
/// бы делать `tired` и `critical` долями `max`, и у каждого кота были бы свои
/// сутки.
///
/// Ноль в `walk` и `idle` значит «как `drain`» — общее правило нулей в рулсете
/// (§12.52, §12.90): мир, где про ставки не сказано, ведёт себя как до §12.99.
///
...
/// Лежащий — не исключение из этого правила, а другое дело: он не бодрствует.
fn tire_rate(needs: &NeedRules, stats: &StatRules, cat: Option<&Stats>, toil: Toil) -> i32 {
    if toil == Toil::Lying {
        return 0;
    }
    let base = match toil {
        Toil::Walking if needs.walk > 0 => needs.walk,
        Toil::Idle if needs.idle > 0 => needs.idle,
        _ if needs.drain > 0 => needs.drain,
        _ => TIRE_PER_TICK,
    };
    let step = stats
        .index_of(STAT_GRIT)
...
pub(crate) fn release_work(
    ...
    // У темы гасим **только своего исполнителя**: ячейка лаборатории принадлежит
    // самой теме, а не задаче кота (§12.132) — дословно как ячейка станка у
    // заказа, — а места заняты другими учёными, которых сон этого не касается
    // (§12.163).
...
/// **чистой**: с §12.99 лежащий кот не тратит бодрости вовсе — ни дремлющий, ни
/// спящий, — и лестница ставок в `tire` знает об этом сама. До §12.99 расход
/// снимался и с дремлющего, отчего у полного кота бодрость колебалась на очко;
/// теперь она просто стоит.
pub(crate) fn doze(
    map: Res<BaseMap>,
    rules: Res<TileRules>,
    needs: Res<NeedRules>,
...
/// Бодрствование стоит бодрости, и **ставку выбирает то, чем кот занят**
/// (§12.99): работа дороже ходьбы, ходьба дороже простоя, лежащий не платит.
/// Кроме тех, кто на миссии: вне базы усталость не считается вовсе (§12.22).
///
/// Занятость разбирается **тем же `Busy::of`, что и подпись в карточке**
/// (§12.41), и это главное в решении: второй перечень задач ради расхода стал
/// бы тринадцатым местом, которое однажды забудут, и новая работа молча стоила
/// бы как простой (§12.52, инвариант 7). Заодно механика получается видимой без
/// единой новой цифры в панели: игрок читает слово, которым она и посчитана.
///
/// Запрос собран **вложенным кортежем** по той же причине, что и в снапшоте:
/// задач ровно столько, сколько берёт `Busy::of`, и плоский упёрся бы в предел
/// арности `QueryData`.
///
/// **На пустой желудок бодрость горит в `starve` раз быстрее** (§12.36). Это
/// вся цена голода: он стоит сил, а не жизни, — и берёт их существующей шкалой,
...
/// Лежащий при этом не платит и голодом — продолжение «спящего голод не
/// будит»: наказание, из которого не выйти сном, §12.20 отверг.
///
/// `Without<Rest>` в фильтре нет намеренно: «лежит — не тратит» это строка
/// лестницы, а не исключение из неё, и одной строкой она накрывает разом сон
/// и дремоту.
pub(crate) fn tire(
    map: Res<BaseMap>,
    rules: Res<TileRules>,
    ...
    mut cats: Query<
        ...
        (With<UnitId>, Without<Away>),
    >,
) {
    for (pos, mut energy, fed, cat, tasks) in &mut cats {
        let (
            order,
            path,
            assignment,
        ...
        // Дремлет тот, у кого нет ничего, а под лапами место для сна (§12.52).
        // Карта и правила тайлов в `Busy` не ходят, поэтому признак считает
        // вызывающий — ровно как в снапшоте.
        let bed = rules.rest_of(map.tile_at(pos.x, pos.y)) > 0;
        let busy = Busy::of(
            order,
            path,
            assignment,
            ...
            squad,
            duty,
            None, // `Away` отсечён фильтром запроса: вне базы усталости нет
            bed,
        );
        let starving = fed.is_some_and(|f| f.0 <= 0);
        let rate = tire_rate(&needs, &stats, cat, Toil::of(&busy))
            * if starving { food.starve.max(1) } else { 1 };
