...
pub(crate) struct TileDef {
    ...
    /// Узел связи: держит одну вылазку (§12.59). Десятое свойство той же схемы и
    /// **вторая лицензия** после торгового поста: за узлом никто не работает,
    /// он только считается — сколько узлов, столько отрядов база выводит разом.
    #[serde(default)]
