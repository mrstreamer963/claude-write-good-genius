...
///
/// Кого послать, рулсет не описывает: отряд набирает симуляция (§12.22).
/// Требований к навыкам и риска провала здесь тоже нет — они придут вместе с
/// авторасчётом исхода (§11, шаг 4b).
#[derive(Debug, Deserialize, Clone, Serialize)]
pub(crate) struct MissionDef {
    pub(crate) id: String,
    #[serde(default)]
    pub(crate) label: String,
    /// Сколько котов уходит; меньшим составом отряд не тронется.
    ...
    pub(crate) ticks: i32,
    /// Добыча: `{ предмет: сколько }`. `BTreeMap` по той же причине, что и у
    /// цены тайла, — порядок обхода должен быть детерминирован (§12.21).
    #[serde(default)]
