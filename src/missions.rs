...
pub(crate) fn run_missions(
    ...
    for (mission_e, mut mission) in &mut missions {
        ...
        if squad.iter().any(|&(.., away, _, _)| away) {
            ...
            let force: i32 = squad.iter().map(|&(.., f, _)| f).sum::<i32>()
                + relay_force(mission.covered, mission.span);
            // Опасность — та, какую встретил этот отряд: проводник режет её
            // делением (§12.70). Считается тем же выражением, что и в прогнозе
            // для панели: игрок обязан получить то, что ему показали.
            let guide = squad.iter().map(|&(.., g)| g).max().unwrap_or(0);
            let out = outcome(raid_danger(rule.danger, guide), force);
        ...
        // Уходим, когда все, кого взяли, стоят на шлюзе: недобор здесь — это не
        // «пошли вдвоём вместо троих», а «ещё идут».
        //
