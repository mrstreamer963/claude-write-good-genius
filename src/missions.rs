...
pub(crate) fn run_missions(
    ...
    items: Res<ItemRules>,
    // Порог ранения нужен здесь ровно за одним: пленный уходит в плен
    // выбывшим — не мог идти, потому и остался (§12.40).
    hurts: Res<HealthRules>,
            ...
            for &(cat_e, ..) in &squad {
                ...
                if let Ok((.., energy, _, health)) = crew.get_mut(cat_e) {
                    ...
                    if let Some(mut health) = health {
                        ...
                        // **Плен — следствие раны, а не отдельное наказание**
                        // (§12.40). Кота оставили там не потому, что с ним
                        // что-то сделали, а потому, что он не мог уйти со
                        // всеми, — значит рана на нём обязана быть видна, чего
                        // бы ни было записано у вылазки в `harm`. Иначе
                        // получается кот, который цел, но почему-то не дошёл:
                        // объяснение, которого игрок в игре не найдёт.
                        //
                        // Хуже, чем есть, при этом не делаем: берём меньшее из
                        // «что осталось» и порога. Плен добирает до выбывшего,
                        // а не бьёт сверху — бить его будет лазарет, временем.
                        if Some(cat_e) == captive && hurts.hurt > 0 {
                            health.0 = health.0.min(hurts.hurt);
                        }
                    }
                }
