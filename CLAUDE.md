...
- **`src/`** — ядро по модулям; `lib.rs` держит только крейт-док, сборку модулей и `pub use Sim`.
  ...
  `skills` (навыки: рост от работы и скорость работы).
...
work_hauls → work_jobs → retry_orders → escape_voids → settle_stacks → train_skills`.
...
10. **Лом не исчезает с пола «в лапы».** Кот поднимает кучу, только если знает, куда её деть, и
...
57 тестов живут в `src/tests/` по механикам (`paths` · `voids` · `orders` · `jobs` · `demolition` ·
`hauling` · `tidying` · `skills`); общие хелперы и сборка мира — в `tests/mod.rs`. Мир собирается
из ASCII-схем, минуя YAML:
...
- Тайл в `sim_from` **бесплатен и не склад** (`TileRules` = нули): цена и ёмкость — контент
  ...
  (`SkillRules` пуст) и предела лап: работа идёт с базовой скоростью, `BUILD_TICKS` тиков на тайл.
- Два теста ходят на боевой `core.yaml` — `the_shipped_ruleset_tidies_demolition_scrap` и
  `the_shipped_ruleset_grows_skills_and_deals_paws`. Они ловят рассогласование кода и контента
  (пропавшую `capacity`, склад без прохода, навык под другим `id`, забытый `carry`), которого
