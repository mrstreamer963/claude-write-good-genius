...
function raidCard(i, node) {
  ...
  const fast = raids[i]?.span_fast;
  if (slow != null && fast != null) {
    if (slow === fast) {
      // Весь срок — дорога. Говорим это словом: игрок иначе будет добирать
      // котов в надежде ускорить то, что не ускоряется.
      facts.push(`срок ${spanText(slow)} — от состава не зависит`);
    } else if (empty) {
      // Число котов в подписи — из вилки самого заказа, а не словом «впятером»:
      // предел у каждого заказа свой, и в рулсете он меняется.
      facts.push(`срок ${spanText(slow)}, отрядом из ${g.most} — ${fast}`);
    } else {
      facts.push(
        `срок ${spanText(g.span)}` +
          (g.paws < g.most ? ` · отрядом из ${g.most} — ${fast}` : ""),
      );
    }
  }
  facts.push(
