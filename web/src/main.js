...
//
// Пост и склад сюда не входят вовсе, и это **не** «ведут в окно» (§12.101).
// Раздел тулбара раскрывается **рядом** с панелью клетки, а модальное окно
// накрывает и карту, и саму панель — то есть стирает ровно тот ответ, за
// которым игрок кликал. Клик по клетке отвечает «что здесь», и точка; склад
// открывают кнопкой в тулбаре и фишками в шапке.
//
...
// порог снимается там, где задавался, — строкой «держать N» в разделе
// «Производство», и ноль там и есть снятие (§12.65).
//
...
function craftCell(snap, x, y) {
  ...
  if (!c) {
    return [
      '<div class="cat-sub">станок свободен — заказы в разделе «Производство»</div>',
    ];
  }
  const pct = c.total > 0 ? Math.round((c.progress / c.total) * 100) : 0;
  // Три разных «ничего не происходит», и путать их нельзя: некому взяться,
  ...
  const foot = c.auto
    ? '<div class="cat-sub">по порогу — снимается в разделе «Производство»</div>'
...
function buildToolbar() {
  ...
  if (recipes.length) {
    ...
    recipes.forEach((r, i) => {
      ...
      // (места на «×N» в одну не хватает), и без обёртки перенос рвал бы её
      // пополам, оставляя цветной квадратик в строке одного.
      const b = mkTool(
        `<span class="rhead"><span class="sw sw-shop"></span>` +
          `<span>${esc(r.label || r.id)}</span></span>` +
...
// Подпись «×N» у рецептов: сколько уйдёт **прямо сейчас**, с учётом зажатого
// Shift. Отдельно от `syncRecipeButtons`, потому что от снапшота не зависит
// вовсе — только от клавиши, — и звать её надо на каждое нажатие и отпускание,
// а не раз в кадр.
//
...
function syncRecipeButtons(list) {
  ...
  recipeButtons.forEach((b, i) => {
    const r = (list ?? [])[i];
    if (!r) return;
    // `shop` значит «есть куда поставить»: ячейка станка — свободная или
    // отбираемая у правила-порога (§12.97), — **или** уже размеченный заказ на
    // этот рецепт, тогда клик добавит штук (§12.96). Своя работа не запирает
    // себя же: все станки заняты деталями — деталей закажут ещё. Считает всё это
    // ядро (`spare_shop_cell`), второго экземпляра здесь быть не должно.
    const ready = r.unlocked && r.shop;
    // ⚠️ Гасим **классом**, а не `disabled` (§12.53, §12.71): по выключенному
    // элементу браузер не шлёт событий мыши, и подсказка ниже не показывалась
    // бы **никогда** — а с §12.96 «все станки заняты» стало частым ответом,
