...
// Та же отмена в панели «Заказы», но целится она в **пачку** (§12.164):
// карточка там одна на рецепт, и адресатов у неё столько, сколько ячеек. Команда
// у ядра при этом прежняя — `cancelCraft` по клетке (§12.96), — и второй её
// формы («отменить весь рецепт») заводить не на чем: заказы различает ячейка, а
// не рецепт. Клетки едут строкой в самой кнопке: панель перерисовывается каждым
// кадром целиком, и реестр рядом с ней разошёлся бы с разметкой на первом же
// закрывшемся заказе. Два `onPanelClick` здесь не дубль правила, а две панели:
// слушатель у каждой свой, и делить его нельзя.
onPanelClick(craftEl, ".crafts-cancel", (node) => {
  for (const cell of (node.dataset.cells || "").split(";")) {
    const [x, y] = cell.split(",");
    sendAction({ type: "cancelCraft", x: Number(x), y: Number(y) });
  }
});
...
function renderCraftPanel(list) {
  ...
  for (const g of craftGroups(orders)) {
    ...
    // Ключ у кнопки — пачка (рецепт и чей заказ), а адресаты едут списком
    // клеток: команда у ядра одна и та же, `cancelCraft` по ячейке (§12.96), и
    // второй её формы заводить не на чем.
    const foot = g.auto
      ? '<div class="cat-sub">по порогу — снимается в окне «Склад»</div>'
      : `<button class="tool crafts-cancel" data-key="crafts@${g.def}" ` +
        `data-cells="${g.cells.map((c) => `${c.x},${c.y}`).join(";")}">` +
        `<span>Отменить${g.orders > 1 ? " все" : ""}</span></button>`;
    parts.push(
      '<div class="cat-skill">' +
