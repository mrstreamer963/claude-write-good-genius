...
function syncStockWindow() {
  ...
  for (const r of wareRows) {
    ...
    for (const k of r.keeps) {
      ...
      // будущий лом. Сколько заявка примет, считает **ядро** (`room`) — тем же
          ...
          : !rs.shop
            ...
            : room > 0
              ? `Произвести: клик — штука, Shift — пять, но не больше ${room} — ` +
                `на столько сейчас хватает материала`
              : `Материала не хватает даже на штуку: ${payHint(
    ...
    for (const k of r.salvages) {
      ...
      // общие, и мерят они кучи — станок снабжают и с пола (§12.102).
      const room = rs.room ?? 0;
      const ready = open && rs.shop && room > 0;
      k.make.classList.toggle("off", !ready);
      k.make.classList.toggle("on", ready);
          ...
          : !rs.shop
            ...
            : room > 0
              ? `Разобрать: клик — штука, Shift — пять, но не больше ${room} — ` +
                `столько сейчас на базе. Обратно не собрать`
              : "Разбирать нечего: этого нет на базе (надетое в счёт не идёт)",
      );
    }
...
function ruleRow(node, at) {
  ...
  const weak = holding && (node.auto_fail || node.auto_share < 100);
  const gathering = holding && !weak && !node.auto_fit;
  const state = blocked
    ? "недоступно"
    : !node.auto_on
      ? "неактивно"
