...
function busyCard(raid, node) {
  ...
  // ⚠️ **Кнопки «Отозвать» здесь нет** (§12.192). Отзыв законен только пока
  // отряд на базе (§12.22), а сбор — это несколько тиков: коты у шлюза, и
  // кнопка появлялась и исчезала быстрее, чем игрок доводил до неё курсор.
  // Кнопка, которую нельзя успеть нажать, — это не решение, а мелькание;
  // тем же доводом §12.170 убрала её из панели «Отряды».
  ...
  // «Отправить» здесь погашена классом, а не `disabled` (§12.71): причину
  // отказа считает ядро (`raidGate`, тот же `busyWhy`), и живёт она в
  // подсказке — «этот отряд её сейчас и ведёт».
  const act = [
