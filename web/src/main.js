...
function renderCraftPanel(list) {
  ...
  for (const g of craftGroups(orders)) {
    ...
    // «×5 ячеек» у заголовка — ответ на «куда делись пять мастерских» (§12.81),
    // и стоит он ровно в том слоте, где до схлопывания стояли координаты: слот
    // и раньше отвечал «какая клетка», теперь — «сколько их». Единица названа
    // словом намеренно: строкой ниже стоит «осталось 50 шт», и голое «×10»
    // рядом с ним читалось бы вторым счётом деталей.
    const many =
      g.orders > 1
        ? ` <span class="pack-n">×${plural(g.orders, "ячейка", "ячейки", "ячеек")}</span>`
        : "";
    // При одном заказе фраза остаётся ровно прежней — той, что говорит и
    // сводка в окне «Склад» (§12.107). Пачка считает ячейки по фазам: числа
    // при одном заказе не пишем, иначе «работает 1» читается как счёт чего-то
    // другого.
    const state =
      g.orders > 1
        ? [
            ...
            .filter(Boolean)
            .join(" · ")
        : craftStateText(g.cells[0]);
    // Кнопки здесь нет ни у одной пачки (§12.165) — вместо неё сказано, где
    // заказ снимают: у заказа правила это порог в окне «Склад», у ручного —
    // сама ячейка, где он и стоит поимённо.
    const foot = g.auto
      ? '<div class="cat-sub">по порогу — снимается в окне «Склад»</div>'
      : '<div class="cat-sub">снимается в ячейке станка</div>';
    parts.push(
      '<div class="cat-skill">' +
        `<div class="cat-row"><span>${esc(def?.label || def?.id || "Заказ")}` +
        `${many}</span><b>${pct}%</b></div>` +
        `<div class="bar"><i style="width:${pct}%"></i></div>` +
        `<div class="cat-sub">осталось ${g.left} шт · ${esc(state)}</div>` +
        foot +
        "</div>",
    );
  }
