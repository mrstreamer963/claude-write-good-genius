...
function fieldLine(e, led, node) {
  ...
  if (node?.guide === e.id) {
    parts.push(`<u>ведёт</u> — сложность −${cut} %`);
    ...
    parts.push(
      `<u>станет проводником</u> — сложность −${cut} %` +
        (now > 0 ? ` вместо −${now} %` : ""),
    );
  } else {
    // Третий случай пишется словом, а не молчанием (§12.71). Молчащая строка
    // читается как «нет данных», хотя это полноценный ответ: кот сложность не
    // изменит. Случаев таких два, и оба непрозрачны без подписи — реакция ниже
    // первого порога (ступень 0, делить нечем) и реакция не хуже, но и не лучше
    // ведущего (ступень грубая: 7 и 5 обе дают первую). Различать их в строке
    // незачем: решение игрока от причины не зависит, а вылазку меняет только
    // тот, кто **побьёт** нынешнего. Тем же молчанием кусались `disabled`-кнопки
    // до §12.71 — правило одно: у отсутствия причины должна быть причина.
    parts.push("сложность не изменит");
  }
