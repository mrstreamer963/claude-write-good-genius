...
function raidCard(i, node) {
  ...
  const facts = [];
  // Срок пустому отряду не показываем — по той же причине, по какой ему не
  // показывают исход (§12.71). Считается он как «дорога + работа на лапу», и на
  // нуле лап работа не делится вовсе: ядро честно возвращает одну дорогу, но
  // для игрока это обещание лучшего случая, который не наступит никогда — хоть
  // один кот пойдёт, и срок вырастет втрое. Ноль на входе не даёт прогноза ни
  // в худшую сторону, ни в лучшую.
  if (g.span != null && !empty) facts.push(`срок ${spanText(g.span)}`);
  facts.push(
