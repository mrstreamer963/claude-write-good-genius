  ...
  const state = !m
    ...
    : m.away
      ...
      : m.resting
        ...
        : "собираются у шлюза";
  // Заказ называется подписью и только пока он есть: у отряда дома его нет, и
  // «—» на этом месте было бы подписью к пустоте (§12.73).
  const job = m ? esc(def?.label || def?.id || "Вылазка") : null;
  const sub = [
    plural((m ? m.squad : node.crew).length, "кот", "кота", "котов"),
    job,
    state,
    verdict,
  ]
    .filter(Boolean)
    .join(" · ");
  // Справа — **докуда дошли**: доля пути у ушедших, слово у остальных. Число и
  // слово в одном месте не спорят: оба отвечают на «скоро ли», просто у отряда
  // дома ответа нет вовсе.
  const mark = !m ? "дома" : m.away ? `${pct}%` : "сбор";
  return (
    ...
    `<b>${mark}</b>` +
    "</div>" +
    // Полоска — только у ушедших и только про путь. У собирающегося отряда пути
    // ещё нет, а вторая полоска «про прогноз» вернула бы в строку тот самый
    // второй смысл, ради снятия которого §12.167 и написана.
    (m?.away ? `<div class="bar"><i style="width:${pct}%"></i></div>` : "") +
    `<div class="cat-sub">${sub}</div>` +
    "</div>"
  );
}

/// Чем занят отряд, который дома. Состояний два, и оба про решение игрока:
/// правило автовылазки (§12.67) отправит его само, а без правила он ждёт
/// приказа. Пустой состав — третье и главное: гараж без котов не отряд, и
/// сказать это надо словом, иначе строка читается как «все ушли» (§12.53).
function homeState(node) {
  if (!node.crew.length) return "состав не набран";
  if (node.auto >= 0) {
    const label = esc(missionLabel(node.auto));
    return node.auto_on ? `повтор: ${label}` : `повтор на паузе: ${label}`;
  }
  return "ждут приказа";
}
