...
function summaryHtml(raid, node) {
  ...
  // Не в сборе — состояние отряда, и место ему здесь, рядом с составом, **один
  // раз** (§12.184): в карточках заказов эта же строка стояла у каждой, хотя
  // причина у них одна и от заказа не зависит. Кого ждать — поимённо из ядра
  // (`unfit`), тем же списком, каким оно гасит кнопку.
  ...
  return (
    ...
    (unfit.length
      ? `<div class="raidwin-why">не в сборе: идут все или никто — ждём ${unfit
          .map(esc)
          .join(", ")}</div>`
      : "") +
    "</div>"
