...
function cellWork(snap, x, y, def) {
  ...
  if (def.trade) {
    // Одна строка — про **эту** ячейку, и больше ничего (§12.81). До этого их
    // было четыре, и каждая повторяла соседа по экрану: роль тайла — заголовок,
    // «за ней не работают» — правило, одинаковое у всех постов навсегда, а
    // «сделок идёт 5 из 28» — шапку панели сделок в двух сантиметрах ниже, теми
    // же двумя цифрами.
    //
    // Оставшаяся строка не дублирует и карточку сделки: та с §12.81 схлопывает
    // одинаковые заказы и клетку больше не называет. Разделение вышло ровное —
    // «что в этой ячейке» спрашивают у ячейки, «сколько всего и на сколько
    // денег» у панели.
    ...
    if (here) {
      ...
      out.push(
        here.buying
          ? `Покупка: ${name} ${here.count} шт · приедет через ${here.left}`
          : `Продажа: ${name} · ${here.delivered} из ${here.count}` +
      ...
      out.push("занята привезённым — пока не вывезут, пост занят");
    } else {
      out.push("свободна");
    }
  }
...
// панели.
//
// Ключ группы — то, что игрок сравнивает глазами и по чему принимал решение:
// сторона, товар, фракция и **зафиксированный курс**. Курс в ключе не для
// красоты: он и есть предмет решения (§12.44), и два заказа по разной цене —
// это две разные сделки, как бы одинаково они ни выглядели.
//
// А вот **фаза, размер заказа и таймер в ключ не входят**. Соблазн был обратный
// — раз сроки разные, пусть и карточки будут разные, — но заказы, сделанные
// подряд, расходятся на десяток тиков, и ключ по таймеру возвращал бы ровно ту
// стену, ради которой схлопывание и заведено: двадцать восемь карточек,
// отличающихся третьей цифрой. Срок из карточки поэтому убран **совсем**, а не
// сжат в вилку: вилка «через 532–562» честна, но отвечает на вопрос, которого
// к пачке сделок не задают. Точный срок — свойство одной сделки, и спрашивают
// его у её ячейки, где он и стоит числом.
//
// Вместо срока карточка считает **контейнеры**: сколько отгружено и сколько
// ещё набирают. Это то же деление на фазы, но числом внутри одной карточки, а
// не тремя карточками, — и оно как раз про пачку.
//
...
function dealGroups(deals) {
  ...
  for (const d of deals) {
    const key = [d.buying, d.item, d.faction, d.unit].join("|");
    ...
    // `delivered == count`, и общая полоска складывается сама собой.
...
function renderDealPanel(list) {
  ...
  for (const g of dealGroups(deals)) {
    const item = (meta.items ?? [])[g.item];
    const who = (meta.factions ?? [])[g.faction];
    const name = esc(item?.label || item?.id || "товар");
    // «×5 сделок» стоит у заголовка, а не в подписи: это первое, что отличает
    // карточку от соседней, и оно же объясняет, куда делись пять ячеек.
    const many = g.deals > 1 ? ` · ×${g.deals}` : "";
    const rows = [
      `<div class="cat-row"><span>${g.buying ? "Покупка" : "Продажа"}: ${name}${many}</span></div>`,
      `<div class="cat-sub">${esc(who?.label || "—")} · ${g.total} шт по ${g.unit} = ${g.unit * g.total}¤</div>`,
    ];
    if (g.buying) {
      // Покупке нечего показывать, кроме того, что она едет: контейнер набивает
      // продавец, а не коты, и делать с этим нечего. Когда приедет — свойство
      // одной сделки, и стоит оно в её ячейке (§12.81).
      rows.push('<div class="cat-sub">в пути</div>');
    } else {
      // Полоска складывает **все** контейнеры пачки: у отгруженного
      // `delivered == count` (иначе таймер бы не пошёл), так что сумма и есть
      // «докуда дошла вся партия». Денег не показываем — их ещё нет: за продажу
      // платят разом по истечении отгрузки (§12.68).
      ...
      if (g.filling) {
        ...
        // «Сколько уже свезли» имеет смысл, только пока что-то набирают: у
        // целиком отгруженной пачки это всегда «40 из 40».
        state.push(`${g.delivered} из ${g.total}`);
      }
      rows.push(
        `<div class="bar"><i style="width:${pct}%"></i></div>` +
...
//
// Начинаем со свёрнутого: палитра теперь останавливает время (§12.74), и
// раскрытая с первого кадра она означала бы игру, которая на каждом обновлении
// страницы встречает игрока паузой, о которой он не просил. Свёрнутое начало
// стоит одного клика — и этот клик и есть «хочу строить».
...
// это потерянные тики, о которых он не просил. Пауза эта **не «наша»**
// (`autoPaused` остаётся снятым): её ставил не раздел, значит и раздел её не
// снимает — уходит она пробелом или кнопкой темпа, как любая пауза игрока.
// `lastSpeed` при этом уже ×1, так что пробел пускает время туда, где оно и
// было бы.
setSpeed(0);
