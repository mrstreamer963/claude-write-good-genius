...
function crewList(snap, x, y, full) {
  ...
  // Четыре группы с подписями — только в штабе. Вопрос «почему сила 0, когда в
  // отряде трое» рамка на коте закрывает наполовину: она объясняет одного кота,
  // а список по-прежнему выглядит одним отрядом. Подписи отвечают на него сразу:
  ...
  //
  // В панели рации подписей нет: колонка вдвое уже, там и так три строки на
  // кота, а порядок групп тот же — читается он и без заголовков.
  ...
  if (!full) {
    return [
      ...
      `<div class="crew-list">${rows.map((r) => r.html).join("")}</div>`,
    ];
  }
  const groups = [];
...
// Каков кот в поле — одной строкой под именем (§12.71). Только в штабе: в
// панели рации колонка вдвое уже, и третья строка сжала бы имя до многоточия.
//
...
function renderRaidWindow() {
  ...
  const left =
    ...
    summary +
    crewList(lastSnap, node.x, node.y, true).join("") +
    "</div>";
