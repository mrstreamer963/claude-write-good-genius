...
function syncRecipeButtons(list) {
  ...
  recipeButtons.forEach((b, i) => {
    const r = (list ?? [])[i];
    if (!r) return;
    // `shop` значит «есть куда поставить»: свободная ячейка станка **или** уже
    // размеченный заказ на этот рецепт — тогда клик добавит штук (§12.96).
    // Своя работа не запирает себя же: все станки заняты деталями — деталей
    // закажут ещё, а вот чужому рецепту вставать некуда.
