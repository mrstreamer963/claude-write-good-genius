...
function crewList(snap, x, y, full) {
  ...
  const rows = cats.map((e) => {
    const mine = here(e);
    // Группа кота — та же тройка, что и `rank`, но с учётом готовности: идут,
    // зачислены и не идут, чужие. Считается здесь, чтобы порядок и заголовки
    // брались из одного места: разойдись они — кот оказался бы под чужой
    // подписью, а это хуже, чем список без подписей вовсе.
    const band = !mine ? 2 : ready.has(e.id) ? 0 : 1;
    ...
    const pick =
      ...
      `${idle ? ` title="Сейчас не пойдёт: ${esc(note || "занят")}"` : ""}` +
      ">" +
  ...
  // Три группы с подписями — только в штабе. Вопрос «почему сила 0, когда в
  ...
  const heads = node?.busy
    ? ["Отряд узла", "Отряд узла", "Прочие коты базы"]
    : ["Идут", "В отряде, но не пойдут", "Прочие коты базы"];
  if (!full) {
    return [
