  ...
  scrapEl.innerHTML =
    (meta.items ?? [])
      .map((it, i) => {
        ...
        // нужны два-три; закладка уже отвечает на «что мне сейчас важно», и
        // второго списка ради шапки заводить нельзя. Ничего не отмечено — шапка
        // показывает всё: пустая строка ресурсов читалась бы как «склад пуст».
        if (favorites.length && !favorites.includes(i)) return "";
