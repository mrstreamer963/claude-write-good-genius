...
function fieldLine(e, led, node) {
  const parts = [`<b>+${e.raid_force ?? 0}</b> сила`];
  ...
  if (node?.guide === e.id) parts.push("<u>ведёт</u>");
  else if ((e.guide_step ?? 0) > led) parts.push("<u>станет проводником</u>");
  // Остальные ступени — по палитре, без единого `id` в коде: параметр без
  // ступени механику не двигает (§12.70) и в строку не идёт, а какие ступени
  // вообще бывают — дело рулсета.
  //
  // Пишем **ступень, а не сырое значение**, и это не украшение. Ступеней у
  // «Реакции» две (5 и 9), поэтому кот с семёркой и кот с пятёркой ведут отряд
  // одинаково — а рядом стоящие «7» и «5» обещают разницу, которой нет, и
  // игрок идёт искать её перебором ровно так же, как раньше. Сырое значение
  // остаётся под наведением: оно объясняет пределы навыков (§12.42), но
  // механику двигает не оно.
  (meta.stats ?? []).forEach((st, i) => {
    const step = e.stat_steps?.[i] ?? 0;
    if (step > 0) {
      const most = (st.steps ?? []).length;
      parts.push(`${esc(st.label || st.id)} ${step}/${most}`);
    }
  });
  return `<i class="crew-field">${parts.join(" · ")}</i>`;
}

// Врождённое сырыми числами — под наведением на строку кота (§12.71). Ступени
// решают, а значения объясняют, почему у этого кота такой потолок навыка.
