...
function raidGate(i, node) {
  ...
  const ready =
    !taken && known && welcome && !nobody && paws >= least && paws <= most;
...
function ruleRow(node, at) {
  ...
  // то спит, ранен или ещё в поле), а недобор — решением игрока, который сам
  // вычеркнул кота из отряда. Обе считает ядро (`auto_fit`, `auto_enough`), и
  // числа для слова приезжают оттуда же: сравнивать `ready.length` с вилкой в
  // JS значило бы завести второй экземпляр той проверки, по которой правило и
  // решает, идти ли (§12.113).
  //
  // Пока отряд в поле, спрашивать не о чем: `busy` вытесняет обе причины.
  const holding = !blocked && node.auto_on && !node.busy;
  const short = holding && !node.auto_enough;
  const gathering = holding && node.auto_enough && !node.auto_fit;
  const state = blocked
    ? "недоступно"
    : !node.auto_on
      ? "неактивно"
      : short
        ? `ждёт ${node.auto_need} котов`
        : gathering
          ? "ждёт сбора"
          : "автовылазка";
  const hold = short
    ? `Правило уводит только полный отряд: заказу нужно ${node.auto_need}, ` +
      `готов${node.ready.length === 1 ? "" : "ы"} ${node.ready.length}. ` +
      `Впишите кота в отряд в штабе — или отправьте вручную оттуда же, ` +
      `недобором: цену вы там увидите`
    : gathering
