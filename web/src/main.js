...
function renderMissionPanel(list) {
  ...
  const parts = [];
  // Сколько слотов занято — там же, где видно сами вылазки. Число гаражей
  // считает ядро (§12.59, §12.152): «вылазок меньше, чем гаражей» вторым
  // экземпляром в JS однажды разойдётся с `launch`.
  if (gates > 1) {
    parts.push(
      `<div class="cat-name">Вылазки · ${raidsOut.length} из ${gates}</div>`,
    );
  }
...
// Одна карточка вылазки. Их столько, сколько гаражей (§12.59, §12.152), поэтому
// у кнопки отмены обязателен `data-def`: `onPanelClick` различает одинаковые
...
function missionCard(m) {
  ...
  if (m.away) {
    // Дежурных зовут по именам: «связь держат» не говорит, кого игрок потеряет
    // из работы и кого нельзя трогать. С §12.152 их может быть несколько и
    // сидят они где угодно — связь общая, своей рации у вылазки больше нет, —
    // поэтому ищем всех, кто дошёл до места и держит эфир.
...
function renderRaidsSection() {
  ...
  raidsEl.innerHTML =
    ...
    nodes
      .map((node, n) => {
        // Номер отряда — по порядку гаражей из ядра, а он row-major по карте
        // (`gate_cells`) и потому детерминирован. Своего имени у отряда нет
...
function syncDoors(snap) {
  ...
  if (sciDoor) {
    // Изучено всё — прячем, по тому же доводу, что и «Найм» (§12.154):
  ...
  if (hireDoor) {
    ...
    // (§12.154). Дверь гаснет там, где правда — «этого **пока** нет»: наука
...
function renderRaidWindow() {
  ...
  // Гараж могли снести, пока окно открыто: клетка пропала — закрываемся, а не
...
function summaryHtml(raid, node) {
  if (raid?.away) {
    return (
      ...
      `<div class="cat-sub">${
        [...raid.squad.map(esc), ...commsPart(raid)].join(" · ") ||
        "в отряде никого"
      }</div>` +
      "</div>"
    );
